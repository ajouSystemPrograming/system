...
//        return 3;
//    }
    //buttonState += GPIORead(SPIN)<<4;
    //printf("%d %d %d %d\n",GPIORead(UPPIN),GPIORead(DOWNPIN),GPIORead(LEFTPIN),GPIORead(RIGHTPIN));
    //printf("BUTTONSTATE:%d U:%d D:%d L:%d R:%d SELECT:%d\n", buttonState, buttonState&1,(buttonState&(1<<1))>>1,(buttonState&(1<<2))>>2,(buttonState&(1<<3))>>3,(buttonState&(1<<4))>>4);
    return buttonState;
}
